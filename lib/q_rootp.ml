...
(* 共役（複素構造ファンクタに渡すために必要。この体自身は実数の範囲に
   とどまるので、共役をとっても自分自身のまま） *)
let conj (x: t): t = x
...
(* Vector_space.Sを丸ごとincludeするとC.map2由来のadd/sub/neg/equal/zeroが
   上の実装を上書きしてしまうため、mul_scalar/div_scalarだけを狭くincludeする *)
    ...
    BasisMap.fold
      (fun set _ acc ->
        if PrimeSet.is_empty set then acc
          ...
          | Some m -> Some (if Integer.compare max_in_set m > 0 then max_in_set else m))
      x None
