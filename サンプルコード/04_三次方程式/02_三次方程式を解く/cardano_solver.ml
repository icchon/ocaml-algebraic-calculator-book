...
module type ROOT_SET = sig
  ...
  val roots : C.t list
  (* Cが呼び出し側からは抽象型（実行時にしか具体的な型が決まらない）
     なので、文字列化の手段もC自身が一緒に持ってきてもらう必要がある *)
...
let solve (poly : Q_poly.t) : (module ROOT_SET) =
  ...
  let a = (List.nth poly 3) in
  let shift_o =
    Rational.div (Rational.neg b) (Rational.mul (3, 1) a)
    |> Omega_field.of_rational
  in
  ...
  let disc =
    Rational.add (Rational.mul half_q half_q) (Rational.pow 3 third_p)
  in
  let sqrt_disc = Omega_field.sqrt_disc disc in
  let u_formula =
    Omega_field.add
      (Omega_field.neg (Omega_field.of_rational half_q)) sqrt_disc
  in
  (* uは、t^2+qt-(p/3)^3=0というレゾルベント方程式の2つの根のうち
     一方。この根が0になるのはp=0のときだけだが、その場合
     alpha=cbrt(u)=0となり、alpha*v=-p/3の関係が0=0で自明になって
     しまい、vを決められなくなる。この場合は、レゾルベントのもう一方の
     根-qを改めてuとして使う（2根は対称なので、どちらを使ってもよい） *)
  let u =
  ...
  | Some alpha ->
      (* パターンA: Omega_field内で解ける場合。ダミーの拡大体を経由する
         理由は後述（2つの型を1つにまとめる） *)
      let module F = struct let u = Omega_field.one end in
      let module C = Cubic.Make(F) in

      (* v = -p / (3*alpha)。alpha=0になるのは、上でuを選び直しても
         なおu=0のまま、つまりp=q=0（3重根0）のときだけで、
         このときはv=0が正しい *)
      let v =
        if Omega_field.equal alpha Omega_field.zero then Omega_field.zero
        else
          Omega_field.div
            (Omega_field.neg (Omega_field.of_rational third_p)) alpha
      in

      let y1 = Omega_field.add alpha v in
      let y2 =
        Omega_field.add
          (Omega_field.mul alpha omega) (Omega_field.mul v omega2)
      in
      let y3 =
        Omega_field.add
          (Omega_field.mul alpha omega2) (Omega_field.mul v omega)
      in
  ...
  | None ->
      ...
      let v =
        C.mul
          (embed (Omega_field.neg (Omega_field.of_rational third_p)))
          alpha_inv
      in
