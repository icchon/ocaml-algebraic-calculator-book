...
let () =
  ...
  show "real roots" [Rational.of_integer 1] [(2,1); (-3,1); (1,1)];

  (* 既約2次（複素根）: 1/(x^2+1) *)
  show "complex roots" [Rational.of_integer 1] [(1,1); (0,1); (1,1)];

  (* 3次以上が残る: 1/(x^3+x+1) （有理根なし、既約3次） *)
  show "degree >= 3 remains" [Rational.of_integer 1] [(1,1); (1,1); (0,1); (1,1)];

  (* 重根: 1/(x-1)^2 = 1/(x^2 - 2x + 1) *)
  show "repeated root" [Rational.of_integer 1] [(1,1); (-2,1); (1,1)]
